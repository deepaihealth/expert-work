...
Measured inside the real sandbox image (linux/amd64 under emulation on an
arm64 host, `docker run --entrypoint python`, three runs each):
  ...
  - 300-page docx (this case): ~4.55-4.57s wall clock across three runs,
    comfortably above the 1s timeout used below.
...
check(
    raised,
    "expected OfficeTimeout with 1s timeout (cold soffice start + 300-page render takes longer)",
)
time.sleep(2)
