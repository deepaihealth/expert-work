...
> **PR6 的真栈验证已经做完了(2026-09-13,测试环境),不用发布当晚再验。**
> 做法是 A/B:先在探针用户的**用户根**上放一个 agent 目录里没有的文件,再用同一个
> 探针、同一句指令跑两次 ——
> **A(有回落)读到了那个文件**,**B(PR6)`read_file failed: not_found`**,
> 且标记串在整个事件流里一次都没出现。
>
> A 那一跑的副产物值得单独看一眼:**它实证了「搬迁之后回落 = 跨 agent 读洞」不是
> 理论** —— agent 真的读到了不属于它的文件。这就是 Step F 存在的理由。
>
> 测试环境目前就停在 PR6 分支构建上泡着,合并后它自然对齐。

- [ ] Step C 的 5+2 条判据全过,且 `shared/` 与 `agents/` 的分布可解释
- [ ] 合并 PR6
- [ ] **PR6 不在本次发布里** —— 它下一班车才上生产。本次发布的生产环境**仍带回落**,
      这是有意的:万一搬迁中途卡住,历史文件还读得到
- [ ] 合并后**发一次测试环境**让它回到 main 构建(合并前测试跑的是分支构建),
      泡到下一班车再上生产
