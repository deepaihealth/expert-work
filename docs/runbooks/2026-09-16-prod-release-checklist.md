...
- [x] **依赖 PR 的取舍已拍板（2026-09-13）** —— 按「会不会进本版 B2」分两堆：

      | PR | 落点 | 进本版 B2？ | 处置 |
      |---|---|---|---|
      | #1520 python-deps ×7 | control-plane / orchestrator 运行时 | **会** | **押后**到 09-16 之后 |
      | #1485 admin-ui minor-patch ×2 | admin-ui 镜像 | **会** | **押后**到 09-16 之后 |
      | #1519 codeql-action SHA | 只改 `.github/workflows/` | 不进任何镜像 | ✅ 已合 |
      | #1484 pypdf | 沙箱镜像 | **不会**（Step A 钉死 `e8aac104`） | ✅ 已合 |
      | #1486 weasyprint | 沙箱镜像 | **不会**（同上） | ⏸ CI 待查，见下 |

      理由：**B2 = `git checkout main`**，所以现在合什么就进什么。这次生产发的是
      数据迁移，B2 的 diff 越接近「只有 contract 段」越好 —— 真出事时「是搬迁还是
      7 个依赖」要能一眼分开。押后那两条合起来就是 09-16 之后独立的一趟 deps 车。

- [x] **#1486 weasyprint 告警（`#137 medium`，唯一还开着的一条）已拍板顺延** ——
      **本版明确不带**。它在沙箱镜像里，而 Step A 钉的是 `e8aac104`，不含它；
      **告警真正关掉要等沙箱钉子刷新**（B-59，09-16 之后）。所以它合不合都不影响
      这次发布，可以慢慢查。
      ...
      - 但它的 `Acceptance suite under runsc` **红是会复现的**。我一开始按
        09-10T13:40 那批（同一批把 main 自己也 cancel 了）判成一次性超时，
        **09-13 重跑又是 20m15s 撞上限，所以那个判断站不住**。近期该档成功的跑是
        3m46s / 4m37s / 7m17s / 8m10s / 10m54s，**没有一次接近 20 分钟** ——
        这条分支上有东西是真的在挂。`#1484` 改同一个文件、同一个 workflow，
        10m54s 就过了。**查清楚之前不合**（非必需 check，不卡任何人）。
