...
>   沙箱镜像钉子要从 `621249f6` 换到 **`05b20c8f`**。见下一段的待重钉表。
>   ⚠️ **不是 `ef9e157a`** —— #1623 那次构建在 90 分钟超时上被 kill，镜像**从来没推上 ACR**
>   （`buildx imagetools inspect ...sandbox:ef9e157a` 报 not found）。#1626 把超时抬到 180 之后
>   重跑,14 分钟构建完成并推送，tag 是 `05b20c8f`。
>
...
> | 钉子 | 现值 | 应该变成 | 还差什么 |
> |---|---|---|---|
> | 应用镜像 | `233791e5` | 覆盖 `233791e5..main` 全部 14 个提交的新 sha | 发一次测试环境 |
> | 沙箱镜像 | `621249f6` | **`05b20c8f`**（砍掉 ffmpeg/npm 的那次重烤，已在 ACR） | 同上 |
>
> `233791e5..main` 的 14 个提交（2026-09-20 重新清点）：
>
> | 提交 | 是什么 | 进本班的理由 |
> |---|---|---|
...
> | `128e4194` #1628 | ROADMAP 记 B-88 / B-89 | 纯文档 |
>
