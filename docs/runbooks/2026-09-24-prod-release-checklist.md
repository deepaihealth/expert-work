...
| 本版 tag | **待定** —— 09-17 验收过的是 `42426d31`；B-80 修复合入后要**重发一次测试环境**、真做一次滚动发布验过，再把钉子改成那一版 |
| 区间提交数 | 28（`5775fbf3..42426d31`）**+ B-80 修复**，钉子发布前重算 |
...
- **B-80 滚动发布不再打断在跑的对话**（2026-09-18 追加）：关机时先让在跑的对话自己跑完，
  到点没完的交给别的副本从存档点接着跑；不可安全接管的才收成 `interrupted`。
...
- [ ] **通知对接方**：待审批会话在窗口内先别处理；滚动期间少量对话会换实例接着跑（对外无感，
      但那几分钟 `rollout status` 会比平时久）。B-80 修复后**不再打断在跑的对话**，
      只有卡在不可重跑工具上的极少数会被硬停。
...
2. **发布会打断正在跑的对话（B-80）—— 本版已修**（2026-09-18 加入本班）：
   ...
   - **预期**：多数发布几秒就收完；最坏情形一轮滚动 6～7 分钟。`rollout status` 比平时久
     **不是故障** —— 它在等对话跑完。
   ...
   - **发布后要看的一条**：`kubectl logs` 里 `run.drain_started` / `run.drain_done`
     （`hard_stopped=N` = 有几个等到上限才被硬停）、`run_agent.handed_off`（交接了几个）。
     两者都为 0 说明窗口内本来就没有在跑的对话。
