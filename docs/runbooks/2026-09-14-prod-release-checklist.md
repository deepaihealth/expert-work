...
- [x] **B2 钉死提交 —— 现钉 `5775fbf3`（2026-09-14 定规矩，2026-09-15 重定钉子）**。
      Step B2 是 `git checkout 5775fbf3`，**不是** `git checkout main`，
      **也不再是 `97a2e724`**。

      B1 一直是钉死的（`ca225258`），B2 最初写成浮动的 main HEAD —— 同一份单子两套
      规矩。浮动的代价不在「发布不准」，在于**任何进三个应用镜像的合并都会自动进
      生产**：那是一次隐式的主干冻结，顺带堵死开发和测试环境发布。钉死之后 main 解冻。

      > ⚠️ **09-15 重钉:`97a2e724` → `5775fbf3`。** 原因是 B-60 写入侧隔离
      > (`621249f6`,09-15)**晚于** `97a2e724`(09-14)——而 B-60 正是 09-14 那次
      > 改期的原因。照旧钉子发,等于把促成改期的那个修复漏在外面。
      > `5775fbf3` 是测试环境实际验过的那一版(它与 `621249f6` 之间只有 ROADMAP 与
      > 测试 overlay 的 newTag 记录,**零服务代码**)。
      >
      > **代价要写明**:重钉把原本说好走班车 2 的三批依赖升级一并拉进 B2 了
      > (`#1520` python-deps ×7、`#1487` vitest 5、`#1545` admin-ui minor-patch ×7),
      > 外加 `dd7c6167`(save_artifact 登记前定位文件)。
      > 「B2 的 diff 尽量只有 contract 段」那条诉求因此**不再成立** ——
      > 真出事时「是搬迁、是依赖、还是 B-60」要分三路查,不是两路。
      > 换来的是**发的东西与测试人员验过的逐字节相同**,这笔交换是拍过板的。

      > ⚠️ **发布当晚别慌：测试环境会比 `5775fbf3` 新。** 钉住之后主干继续走,
      > 测试环境跟着走(09-16 已发 `8f0d21d3`,带 B-61 全部)。**两边不一致是预期的**,
      > 不是漏发。要比对生产实况就拿 `5775fbf3` 比,别拿测试环境当下的版本比。

- [x] **依赖 PR 的取舍已拍板（2026-09-13 定诉求，2026-09-14 改手段）** ——
      按「会不会进本版 B2」分两堆：

      > ⚠️ **本表的结论已被 09-15 重钉推翻(上一条)**:B2 从 `97a2e724` 改钉
      > `5775fbf3` 之后,下面前三行标「不会」的三批**实际会进本版 B2**。
      > 原表保留在这里是为了说明当时的取舍,不是执行依据。

      | PR | 落点 | 进本版 B2？(重钉后) | 处置 |
      |---|---|---|---|
      | #1520 python-deps ×7 | control-plane / orchestrator 运行时 | **会**（`b20f0252`,原判「不会」） | 已合 |
      | #1487 vitest 5 | admin-ui 镜像 | **会**（`5e087e5d`,原判「不会」） | 已合 |
      | #1545 admin-ui minor-patch ×7 | admin-ui 镜像 | **会**（`698192b1`,原判「不会」） | 已合 |
...
- [x] **测试环境已泡过 B2 那一版本身**（不是「泡过某个更新的 main」）——
      **重钉后的 `5775fbf3`(服务代码 = `621249f6`)于 2026-09-15 发测试环境,
      SMOKE PASS + 金丝雀 PASS,B-60 真栈验收逐条过,测试人员的对接流程也跑完确认通过。**
      这是本版 B2 的直接演练。

      重钉前的 `97a2e724` 也单独演练过(2026-09-14 已发,SMOKE PASS + 金丝雀 PASS 5/5,
      `CANARY_OK 60875447`,run 28.7s,记录 PR #1546),发的时候 main 已经是 `698192b1`,
      **刻意发的钉子不是 main**。这一条现在只是历史。
      B-50 搬迁 2026-09-13 在测试环境跑通过一次。
...
- [x] **B-61 全部不在本版**(2026-09-16 记)。B-61 的五个 PR(#1557 / #1559 /
      #1561 / #1562 / #1563)都晚于 `5775fbf3`,已合 main 并于 2026-09-16 发测试环境
      (`8f0d21d3`,SMOKE PASS + 金丝雀 5/5),但**真栈验收(T10)一条都还没跑**。
      走班车 2。

      > 这条写出来是因为发布当天最容易犯的错是「main 上有就顺手带上」。
      > 本版 B2 钉的是 `5775fbf3`,`git checkout` 之后 B-61 自动不在里面 ——
      > **只要不把钉子改成 main 就不会出错**。
...
| **B2 contract** | 发 `5775fbf3`（摘掉回落，PR6 + B-60） | 搬完之后回落反过来成了跨 agent 读洞 —— 用户根上剩下的恰恰是**别人的**历史文件 |
...
**发 `5775fbf3`，不是当下的 main，也不是 `97a2e724`。** 理由见 §1:`97a2e724` 早于
B-60(`621249f6`),而 B-60 正是上一次改期的原因;`5775fbf3` 才是测试环境验过的那一版。
它之后合进 main 的东西(B-61 全部)属于班车 2,本版不带。
**测试环境此刻已经比它新(`8f0d21d3`),那是预期的。**

```sh
git fetch origin main
git checkout 5775fbf3            # contract 版本；钉死的，不是「当下的 main」
git log -1 --oneline             # 确认就是它
...
- [ ] `chore(deploy): prod newTag 5775fbf3` PR，正文写上**上一版 tag `ad79ba28`**，
      并注明本次是 **B1 `ca225258` → 搬迁 → B2 `5775fbf3`** 三段
- [ ] ROADMAP 班车 1 销案
