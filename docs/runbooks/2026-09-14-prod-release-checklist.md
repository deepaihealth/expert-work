...
> ⛔ **再次改期（2026-09-14 16:30 拍板）：今晚不发。** 发测试环境后发现 B-50 的写入侧漏洞
> （`exec_python` 写 `/workspace/x` 落用户根，产物登记指向 agent 目录，下载 404；对接方的
> PPT 流程会在生产上直接中招）。用户拍板「不能带着 bug 上线」，先做 **B-60**
> （spec `docs/superpowers/specs/2026-09-14-workspace-exec-mount-namespace-design.md`），
> 做完从新钉子重发测试环境、测试人员验证之后再定生产日期。**下面的 B1/B2 钉子届时重定。**
>
> ~~改期：原定 2026-09-16，用户 2026-09-14 中午改为当天 18:00。~~
> 钉子不受影响 —— B1 `ca225258` / B2 `97a2e724` 都是钉死的提交，与发布日期无关。
> 提前发反而有一项实打实的优势：`97a2e724` 的测试环境演练就在**同一天上午**跑完
> （SMOKE PASS + 金丝雀 PASS 5/5，记录 PR #1546），本机三个 base 刚拉过、镜像缓存是热的。
> 近期发布连炸四次全是本机环境问题（ECR 限流 / 盘满 / 网络），热缓存正是对着这条的。
...
| 本版 tag | **B1 = `ca225258`**(expand,已定) / **B2 = `97a2e724`**(contract,**2026-09-14 钉死**，不再是「发布时的 main」) |
| 区间提交数 | 115（`git log --oneline ad79ba28..97a2e724`） |
...
- [x] **本版装载已确认**（2026-09-14 核过）：`ad79ba28..97a2e724` = **115 个提交**，
      与 ROADMAP 班车 1 条目对齐。`ca225258`（B1）确认是 `97a2e724`（B2）的祖先，
      三段顺序成立。区间内的迁移**正好三个**，与 §3 写的一致：
      ...
      ```
- [x] **发布时段 = 2026-09-14 18:00（用户拍板）**。2026-09-13 查过生产近一个月的
      非金丝雀 run 分布（北京时间）：
