...
**通用型 NAS 没有快照功能**(自动快照策略仅极速型可用),等价恢复面是两件:

- **回收站**(测试盘 2026-07-28 起已启用,保留 7 天):NAS 控制台 → 文件系统 →
  数据保护/回收站。删除类操作全兜——包括 janitor 的 `rm -rf`,每次归档删目录
  都会进回收站躺 7 天,给「rmtree 半途失败覆盖窗口」(见下)多垫一层反悔垫。
- **云备份 HBR 每日备份计划**(每日一次、保留 7 天):覆盖「文件被改坏」这类
  非删除损伤。按保护容量计费——测试环境跳过,生产上线前评估启用。
