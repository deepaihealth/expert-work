...
- 新建要点：
  - 中文字体必须同时设 `w:eastAsia`（只设 `font.name` 中文会回落成默认字体）；给出设置函数。
...
- 新建要点：
  ...
  - 中文字体 `font-family: "Noto Sans CJK SC"`。
...
| 成品在用户电脑上的中文字体（Noto Sans CJK 在 Windows / Mac 上通常没装，会被替换） | **待用户确认的取舍**：见 §8.1 |
| openpyxl 改已有 xlsx 会丢原有图表 | 正文要求先告知用户 |
| soffice 冷启动慢（首次转换数秒） | 超时默认 120s；正文提示合并多次转换 |
| 时间紧 | §7.4 逐技能 go / no-go，不降标准 |

### 8.1 待确认：docx / pptx 成品的中文字体名

- **选项 A（推荐）**：成品里写 `微软雅黑`（Microsoft YaHei）。Windows 与 Mac 版 Office 通常都带这个字体，
  客户打开看到的就是设计好的样子；沙箱里没有雅黑，预览时 LibreOffice 按字体配置自动回落到 Noto Sans CJK，
  版式基本一致。
- **选项 B**：成品里写 `Noto Sans CJK SC`。沙箱预览最准，但客户电脑多半没装，Office 会自行替换字体，
  实际看到的效果不受控。
- PDF 不受影响：weasyprint 把 Noto Sans CJK 嵌入文件，任何电脑都一样。
...
- `docs/superpowers/ROADMAP.md`：登记本项；登记旧版本许可风险（D4）。
