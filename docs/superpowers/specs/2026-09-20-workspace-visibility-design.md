...
* **不改 `list_dir` 的语义**。清单是补充,不是替代 —— 模型仍然要能在一轮之内看到
  ...
  **⚠️ 勘误(09-20 加 §7b 之后)**:这一条原本写的是「不改 `list_dir` 工具」,而
  ...
  参数、返回形状、作用域不变,这才是这一条真正要守的东西。

  **⚠️ 勘误之二(09-20 PR-2b #1648 落地之后)**:上面那句「对模型可见的语义一个字
  不变」**说满了**。实际交付里有**三处窄幅收紧**,都是宿主侧接管带来的,批准归批准,
  但必须写在这里 —— 否则下一个人读到那句话会被它误导:

  1. **`read_file` 不再跟任何 symlink**(宿主侧 `O_NOFOLLOW`)。沙箱片段允许工作区
     **内部**的 symlink(`realpath` 之后仍在根内就放行)。收紧方向,且与既有
     `NasWorkspaceStore.read_file`(产物下载一直在用)一致。**这一条不能"改回去"**:
     `O_NOFOLLOW` 正是让 `openat` 链成立的那个不变式,去掉它逃逸守卫就松了。
  2. **`list_dir` 的 `is_dir` 用 `follow_symlinks=False`**,所以「指向目录的
     symlink」现在报 `is_dir=False`,沙箱片段报 `True`。
  3. **supervisor / 内存两档的 `list_dir` 从扁平清单派生**,因此看不见空目录、也
     看不见保留前缀(它们的清单已被服务端过滤)。只影响本地 dev 与 CI,生产跑
     NAS 实现。

  另有**一处能力回退**:**user-less run 的只读文件操作现在明着拒**
  (`ToolBlockedError`,正文写 "this run has no persistent workspace"),不回落成
  「读到一棵空树」。理由:NAS 按 `{tenant}/{user}` 存,没有 user 绑定的 run 的
  `/workspace` 是沙箱里的临时 tmpfs,宿主侧根本没有那棵树;回落成空树正好是把
  「这个 run 没有持久工作区」伪装成「你的文件不在了」—— 正是本波要消灭的那句误判。
  生产不受影响(对外 API 的 `user_id` 必填,控制台与触发器 run 都有 user);
  受影响的是 eval CLI 一类无 user 场景。**已知不对称**:这类 run 的 `write_file`
  仍走沙箱、能成功,而读会被拒。
...
目录。**实测推翻了这个担心 —— 而且是在真正的那对客户端上测的。**

**第一轮(09-20 上午)测的是 control-plane → control-plane**,12/13/13 ms、内容改动
15 ms。那一轮**不足以支撑本设计**:真实路径的写入方是沙箱(ACS microVM,自己的
NFS 客户端,外加 B-60 每次 exec 的 mount namespace),不是另一个 control-plane pod。
拿「同一种客户端之间」的结果去替「两种客户端之间」下结论,就是在不可能失败的条件
下做验证。

**第二轮(09-20 下午)补测了真实路径。** 测法:用测试环境的 `pf-probe`(带
`exec_python`,走真沙箱),它的 `exec_python` 被策略闸挡住会停在审批 —— 这反而成了
精确的计时锚点,批准的那一刻工具才执行。宿主侧在 control-plane pod 里开紧轮询。
两个时间戳都取自各自进程:沙箱把 `time.time()` 写进文件内容,宿主记录第一次看见的
时刻。

```
新建文件(目录属性缓存):
  沙箱写入   1789889987.5225513
  宿主 listdir 首次看见  1789889987.530886   → 8.3 ms
  宿主 stat    首次看见  1789889987.530935   → 8.4 ms
  宿主读完内容           1789889987.538951   → 16.4 ms

改写已有文件(文件数据缓存,另一个缓存):
  沙箱写入   1789890090.820725
  宿主读到新值           1789890090.835881   → 15.2 ms
```

两轮都是**最坏情况**:宿主在写入发生前已经对该目录连续 `listdir` 15 秒、对该文件
连续 `read` 16 秒,属性缓存与数据缓存都是热的。时钟偏差可忽略(沙箱写入时刻与
NAS 服务端记录的 `mtime` 差 1.7 ms)。

两个缓存分开测是有意的:`list_dir` 吃的是目录属性缓存,`read_file` 吃的是文件数据
缓存 —— 只测前者会漏掉「改写后读到旧内容」这一类,而那正是 `read_file` 会踩的。
...
**余量足够大**:模型 `write_file` 之后要再调一次 `list_dir`,中间隔着至少一个 LLM
往返(秒级)。判据是 8~16 ms 对上秒级,不是压着边跑。

### 顺带逮到的一条:NAS 上的目录名不是 agent 名

探针的 agent 叫 `pf-probe`,它在 NAS 上的目录却是 `pf-probe-33086dc0`。
`sanitize_agent_key()`(`expert_work/protocol/agent_key.py:55`)= 净化后的名字 + 原始
名字 sha256 的前 8 位,后缀是防碰撞用的,**唯一性全在摘要上,不在前缀上**。

宿主侧作用域解析必须一路用 `agent_key`,任何地方手工拼 `agents/<agent 名>` 都会指向
一个不存在的目录 —— 而它的失败形态是「目录是空的」,不是报错,所以测试里很容易看着
像通过。
...
**⚠️ 勘误(09-20,用户追问「那你想挪哪里去」之后去核的)**:这一段原本写的理由是
「`write_file` / `edit_file` 的语义与 B-60 的私有 `/w` 挂载空间、以及工作区写锁绑在
一起,挪它是另一个量级的改动」。**两条都不成立**,去数之后:

* **锁本来就在宿主侧。** `PgWorkspaceLock`(Postgres 顾问锁,跨副本)由 orchestrator
  进程持有,三个调用点都在宿主代码里:`file_ops.py:696`(`write_file`)、
  `file_ops.py:859`(`edit_file`)、`bash.py:145`(`bash`)。写操作换执行地点,这把锁
  一个字不用动。
* **路径计算是同一段。** 宿主侧要算的是 `{root}/{tenant}/{user}/agents/{agent_key}/{rel}`
  —— 与本 PR 为读路径做的作用域解析完全同一套,不是第二套机制。
* 而且**宿主侧的写原语已经存在、已经在生产上跑**:`NasWorkspaceStore.write_file`
  (`nas_workspace_store.py:727`),带 `openat` 链 + `O_NOFOLLOW` + `_LEAF_FILE_MODE`,
  上传接口(`control_plane/api/uploads.py:244`)今天就在用它往工作区写字节。

**真正的拦路项只有一条,而且是新的:挪完之后工作区会有两个 NFS 客户端在写。**
NAS 挂的是 `local_lock=all`,**跨客户端没有文件锁**,序列化全靠那把顾问锁 —— 而
`bash` 持锁、**`exec_python` 不持**(`sandbox.py` 里没有任何 `workspace_lock`)。
今天这不出事,是因为所有写者都在同一个沙箱里,沙箱自己的文件系统替我们串好了;
把一个写者搬到宿主,这层隐式串行就消失了。**这条要先解决**,它的失败形态是静默
写坏,不是报错。

**所以本波不挪写操作,但理由是另一条:它挪不动那个大数。** 67% 的墙钟是模型逐字
敲那 21,173 个字符,字节最后落在哪儿一个生成 token 都不省。挪写买到的是工具侧延迟
(基线 run 里工具总共 123 秒)与写路径的失败形态,是第二序。

**建议的顺序**:本波做读 → 发测试泡一周,看 PR-5 的编辑格式引导把 `edit_file` 16%
的失败率压到多少 → 还高再挪写,那时复用本 PR 已经在生产跑过一圈的作用域解析,并
同时补上 `exec_python` 的锁。让作用域解析先在低爆炸半径的那一侧跑熟:同一套逃逸
用例,读出 bug 是给模型看了不该看的数据,写出 bug 是覆盖掉别人的文件。
