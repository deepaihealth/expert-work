...
**勘误(2026-09-16,文档任务实测)**:上一版把「安全性与今天完全一致」写成了这个方案的通用结论 —— **它只在内联还在时成立**,而 §七 建议的第 2 处模板改动**正是删掉 `materials` 的内联**,围栏跟着一起没。owner 改完模板之后是一好一坏两面:
...
- **`materials` 的请求形状与素材预拉**:见 §七 item 5 与 ROADMAP B-61(此处不复述)。
