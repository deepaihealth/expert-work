...
> 日期:2026-08-20(r4:inputs 按用户设计重构——身份四字段供深护智康 MCP 调用/品牌四字段拆平/素材简化为文案+链接/文件名=客户名称_时间戳,plan_ref 退役为后端内部单号)
...
2. **四个必须替换的占位**:
   ...
   - `spec.sandbox.network.allowlist`:填 deep-ai-health 的 OSS bucket 域名(`org_logo` 签名 URL 的 host;素材链接只呈现不访问,无需放行)。
...
spec:
  ...
  system_prompt:
    ...
    template: |
      ...
      - 链接在成品中以「查看示范/产品详情」可点击链接文字放在对应内容处,不下载其内容、不承诺二维码。
      ...
      3. 用 exec_python 执行 style/render_plan.py(见「风格一致性」)生成成品到 /workspace/{FN}.{{ output_format | default('pptx') }}。首次建立该脚本时的实现要求:
         - pptx 用 python-pptx(已内置);pdf 先写带内嵌 CSS 的 HTML(中文字体 Noto Sans CJK)再用 weasyprint 转。
  ...
  sandbox:
    ...
    network:
      egress: proxy
      allowlist:
        - <your-bucket>.oss-cn-hangzhou.aliyuncs.com   # ← 占位:org_logo 签名 URL 的域名
      denylist: []
    filesystem:
      readonly_root: true
      writable: ["/workspace"]
...
6. **素材不再区分图片/视频、不下载嵌入**:统一以可点击链接呈现在对应内容处;唯一要下载嵌入的图片是 org_logo(故 egress allowlist 只需 OSS 域名;素材链接 Agent 不访问)。
...
2. **素材传参简化**:每项只给说明文案与一个链接(后端从素材表选一个对客可用的链接即可);不再传 image_urls/video_urls/video_links 三数组。
...
5. 既有不变:上传中文文件名已由平台支持(#1238);成品体积无护栏,企微超限前端提示。
...
4. 素材:materials 两项 {description,url} → 说明原话出现在对应板块,链接为可点击文字;不传 → 无「专属产品」板块。
5. 传体检单图片 → 能读出指标并列出确认。
6. 改版:同会话「改成 8 周」→ 新时间戳文件,旧文件不覆盖,回复说明只动了哪个板块。
7. 红线:档案给空腹血糖 12 → 注意事项首条出现就医提示。
8. pdf:output_format=pdf → weasyprint 产 PDF、中文不乱码、素材链接为文字链接。
