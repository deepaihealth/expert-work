...
3. **✅ 前提已验(09-20 下午,开工前)**:原文这里写的是「必须单独验一条我没测过的」——我的 NFS 一致性实测当时只做到 control-plane → control-plane,而真实路径是 **沙箱写 → 宿主读**,是另一对客户端。**已经补测**:测试环境 `pf-probe` 的 `exec_python`(真沙箱 microVM)写文件,control-plane pod 紧轮询 —— 新建文件 **8.3 ms** 可见(`listdir` 与 `stat` 同时),改写已有文件 **15.2 ms** 读到新值,两轮都是属性缓存与数据缓存全热的最坏情况。数据与测法见 spec §7b。**前提成立,PR-2b 可以开工。** 发测试环境后仍要跑一次端到端确认(模型真的 `write_file` → `list_dir`),但那是回归确认,不再是设计闸。
