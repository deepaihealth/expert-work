...
| PR3b exec cwd | 8 | ✅ **已合 #1513(09-12)+ 已发测试环境 `9af710fc`**。做法照行业形状:cwd 是 per-exec 参数(E2B / Daytona / OpenAI 托管沙箱都如此),补的是本地 supervisor 的 `ExecRequest.cwd`。**真栈验收手工补**(契约档在 CI 里没跑成):复用金丝雀温沙箱,三条判据全中。**集群其实只跑云端那半** —— 沙箱 pod 命令被覆盖成 `bash -c sleep`,`runner.py` 不执行;镜像钉子落后 34 天这件事另记 B-57 |
