...
| B-51 | **(2026-09-11 查 D 线时撞见)平台凭据页会让运维得出「不用配」的结论,而平台自己的后台功能正因此空转**。实证:**生产 24h 报 6 次、测试 24h 报 208 次** `memory_consolidator.cluster_failed`,栈底 `tenant_secret_overlay.py:62` → `CredentialsResolverError: platform credentials missing for provider=anthropic`;每次 `sweep_complete` 都是 `consolidated=0` —— **长期记忆整合从上线起就没成功过,两个环境都是**。**配置入口是有的**(`SettingsPlatformConfig` 的 credentials tab,`platform_config.py:316` 按 `PROVIDER_CATALOG` 全量列,未配置的也在,`source="unset"`),所以问题不是没地方配。**问题是那个页面主动误导**:`used_by_agents`(`platform_config.py:312`)**只数 agent**,而平台自身有**四个** provider 依赖点(`settings.py:285 embedding_provider="qwen"` / `:630 eval_agent_provider="anthropic"` / `:651 quality_judge_provider="anthropic"` / `:764 memory_consolidator_default_aux_provider="anthropic"`),**一个都不计入**。于是 anthropic 那行显示「`unset` + `used_by_agents=0`」——「没配,也没人用」,运维据此判断不需要配,完全合理。生产上 eval 与 quality monitor 是关的(`ENABLE_EVAL_WORKER` / `ENABLE_QUALITY_MONITOR` 都 false),所以今天只有记忆整合在撞;**那两个一旦打开就是同样的死法**。**要做三件**:① 凭据页加「平台自身在用」维度(数据现成,就是上面四个 settings 键),与 `used_by_agents` 并列;② `source=unset` 且有平台依赖 → 页面**显式警示**,不能跟「真没人用」长一个样;③ **worker 侧不许静默空转** —— 凭据缺失时要有界面可见的状态(健康位 / 告警),而不是每 4 小时往 ERROR 日志里喊一次没人看。**用户 09-11 拍板:不是去配某个厂商,是把「没配」变成看得见。** |
---
