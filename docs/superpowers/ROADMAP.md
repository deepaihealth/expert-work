...
**顺带记两条**:`{tenant}/.deleted/<user>/`(软删工作区归档)与日备恢复出来的工作区都是**旧扁平布局** —— 今天靠 PR3 的迁移期读回落兜着,**PR6 摘掉回落的那一刻它们会直接看不见**,Task 14 要一并处置。另:金丝雀用户一个 `canary-check.txt` 挂了 **27 个版本行**(每次发版写同名产物),是「行数 ≠ path 数」最直观的例子。**PR6(Task 14 摘掉迁移期读回落)✅ 已写完(#1532)并已在测试环境真栈验证,但刻意不合。**

**闸门**:`read_with_legacy_fallback` 是**生产发版到搬迁完成之间的安全网** —— agent 在自己目录下读不到时回落用户根再读一次。搬迁完成后它反过来成了纯粹的跨 agent 读洞,但**在那之前它是必需的**:今天测试环境的搬迁跑了三轮修 bug 才成,生产同样可能中途卡住,那个窗口里没有回落,agent 连历史文件都读不到。闸门写进了 **09-16 执行单的 Step F**,不靠记忆 —— 它是发布当晚手里那张单子上的一行。**PR6 下一班车才上生产,09-16 的生产环境仍带回落,这是有意的。**
