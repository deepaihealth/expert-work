...
| B-51 | **(2026-09-11 查 D 线时顺带发现)生产 `memory_consolidator` 每 4 小时报一次 ERROR,长期记忆整合从上线起就没成功过**。日志 `memory_consolidator.cluster_failed tenant=b0f0d29b… user=81066c49… size=3`,栈底是 `tenant_secret_overlay.py:62` 抛 `CredentialsResolverError: platform credentials missing for provider=anthropic (tenant-effective view). Configure the platform credential or remove the tenant's disabled override.`。链路:`_consolidate_or_reject` → `aux_model_adapter.__call__` → `resolve_provider`。生产 configmap 配的是 `MEMORY_CONSOLIDATOR_DEFAULT_AUX_PROVIDER=anthropic` / `..._AUX_MODEL=claude-sonnet-4-6`,但平台级 anthropic 凭据在生产**没配或被租户 disabled override 挡住**。**每次 sweep 的 `sweep_complete` 都是 `consolidated=0`** —— 即功能全程空转,而且只在 ERROR 日志里喊,没有任何告警。要定:补平台 anthropic 凭据,还是把 aux provider 换成生产已有凭据的厂商 |
---
