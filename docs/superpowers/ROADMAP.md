...
| B-57 | **(#1513 交付时量出来,2026-09-12)沙箱镜像的 tag 手工钉在 `infra/k8s/sandbox/sandboxset.yaml:90`,不在任何发布路径上 —— 集群跑的镜像已经落后 34 天**。钉的是 `sandbox:63a3109f`(2026-08-09,`chore(deps): bump pypdf` #1126),而此后 `infra/sandbox-image/` 被改过 **4 次**:#1209 / #1377 两次 pypdf 升级、#1402(X-8 官方镜像离开 Docker Hub 改从 ECR Public 拉 + 来源卫兵)、以及 #1513 本身。`sandbox-image.yml` 在 push main 且路径命中时**会自动构建推送**新镜像,但 `release.sh` / `build-push.sh` 只管 control-plane / admin-ui / credential-proxy 三个,**没有任何东西会推进这个钉子** —— 所以那三次依赖升级也从没进过集群。**与记忆里 credential-proxy 那次同形**(`94f1a371` 钉死,B-31② 合了没进任何环境,靠 #1411 修 + smoke 校验才发现)。**对 #1513 本身是良性的**:集群沙箱 pod 的命令被覆盖成 `bash -c "sleep 2147483647"`,exec 由 ACS/envd 做,`runner.py`(镜像 ENTRYPOINT)在集群里根本不执行 —— PR3b 在集群上生效的是云端那半(`mkdir -p && cd`,随 control-plane 镜像走,已真栈验过)。**但对依赖升级不是良性的**:pypdf 两次升级、ECR Public 改源都停在钉子之前。**候选动作**:① 把沙箱镜像纳入 `release.sh`(像 credential-proxy 2026-09-07 那次那样);② 或者给它一条 smoke 校验(比对钉子 tag 与 main 上 `infra/sandbox-image/` 的最新构建),让漂移可见而不是静默;③ 先手工推进一次钉子到 `9af710fc`,把 34 天的欠账清掉。**先记录,未排期** |
---
