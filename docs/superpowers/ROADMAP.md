...
**顺带记两条**:`{tenant}/.deleted/<user>/`(软删工作区归档)与日备恢复出来的工作区都是**旧扁平布局** —— 今天靠 PR3 的迁移期读回落兜着,**PR6 摘掉回落的那一刻它们会直接看不见**,Task 14 要一并处置。另:金丝雀用户一个 `canary-check.txt` 挂了 **27 个版本行**(每次发版写同名产物),是「行数 ≠ path 数」最直观的例子。**下一站 = PR6(Task 14 摘掉迁移期读回落)**,前置已全部满足 |
