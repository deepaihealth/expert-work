...
const zhCN: TranslationKeys = {
  ...
  conversations_detail: {
    ...
    decide_ok: "已提交裁决,run 正在续跑。",
    decide_failed: "裁决提交失败。",
    cancel_run: "取消运行",
    cancel_run_confirm: "取消正在进行的 run?已完成的部分会保留。",
    cancel_done: "已取消运行。",
    cancel_failed: "取消失败。",
    role_user: "用户",
    role_assistant: "智能体",
  ...
  agent_form: {
    ...
    tool_note_behavior_patch_help:
      "让 Agent 记录「以后遇到这种情况该怎么做」的小修正,逐步积累行为改进。",
    ...
    section_dynamic_workers: "动态子 Agent",
    section_dynamic_workers_help:
      "开启(默认)时,Agent 运行中可按需创建临时子 Agent(spawn_worker 工具),分头并行处理子任务;数量与并发受平台「动态子 Agent 护栏」统一约束。\n需要严格单线、只用一个执行体时关掉。\n示例:研究型 Agent 开,简单问答 Bot 关。\n与下方「子 Agent 委派」的区别:名册是预置的具名委派对象,这里是运行中临时造的通用子 Agent。",
    dynamic_workers_hint: "允许 Agent 运行中按需创建临时子 Agent 分担子任务(默认开启)。",
  ...
  console: {
    ...
    thread_id_label: "会话",
    turn_count: "{{n}} 轮",
    no_turns: "还没有对话 —— 在下面输入,或从左侧打开一个会话。",
    row_think: "思考",
    row_think_live: "思考中",
    ...
    detail_variables: "变量",
    detail_worker_steps: "子 Agent 步数",
    ...
    interrupt_reason_agent_disabled: "已中断(Agent 下线)",
    footer_export: "导出",
    turn_view_run: "查看运行",
    toolbar_duration: "时长",
    ...
    detail_schema_missing: "当前 Agent 的工具集里没有这个工具",
  ...
  settings_platform: {
    ...
    dynamic_worker_heading: "动态子 Agent 护栏",
    dynamic_worker_help_title: "什么是动态子 Agent 护栏?",
    dynamic_worker_help_body:
      "spawn_worker 工具的限额:单次运行可同时创建的动态子 Agent 数量上限、单次运行累计可创建的数量上限,以及每个子 Agent 可执行的步数上限。未配置时使用进程的环境变量默认值。",
    dynamic_worker_max_concurrent_label: "单 run 并发上限",
    dynamic_worker_max_per_run_label: "单 run 累计生成上限",
    dynamic_worker_max_iterations_label: "单个子 Agent 步数上限",
    dynamic_worker_env_default: "环境默认",
    dynamic_worker_hint:
      "改后对下一次运行/构建生效,无需重启;未配置时使用环境变量默认值。",
    dynamic_worker_save: "保存",
    dynamic_worker_saved: "动态子 Agent 护栏已保存。",
    dynamic_worker_save_failed: "保存动态子 Agent 护栏失败",
  ...
  api_keys: {
    ...
    scope_help_read: "只读:查询会话、运行结果、Agent 列表等 GET 接口",
    scope_help_write: "业务写:在 read 的只读能力之上,可调用 Agent 运行、创建/继续会话、上传文件",
