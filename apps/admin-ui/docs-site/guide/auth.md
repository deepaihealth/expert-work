...
| `write` | 在 `read` 之上,可以创建/推进会话、发起 run | 调用 [`POST /v1/agents/{agent_code}/runs`](./run-agent) 必须要有它——接口要求调用方具备 session 资源的写权限;[取消 run 与审批决策](./run-control) 这两个端点同样要求 `write` |
