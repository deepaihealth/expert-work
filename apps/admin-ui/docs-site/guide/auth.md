...
创建 key 时选择 0 个或多个权限档位（`read` / `write` / `admin`）。不带任何档位的 key 调用对外接口都会返回 403，对接至少要给 `read`：

| 档位 | 能做什么 | 什么时候给 |
|---|---|---|
| `read` | 只读：Agent 目录、会话列表与历史消息、run 列表与状态、按 run 拉取事件（断线后可续传，见 [3.6 断线重连](./sse-events#_3-6-断线重连与续传)）、工作区文件列表与下载、附件下载、产物列表与下载 | 只查询、不发起新 run 的纯只读集成 |
