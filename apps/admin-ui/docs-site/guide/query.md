...
- 终端用户随消息上传的图片不会出现在 `content` 里。只上传了图片、没有输入文字的用户消息仍然占一条，`content` 为空字符串；要取附件，用 [2.6 带图片和文档](./chat#_2-6-带图片和文档) 的下载接口。
...
| `artifacts` | array \| null | 这次 run 登记过的产物清单，元素结构与 [3.4 的 `end`](./sse-events#end) 完全相同：`{name, kind, version, created_at}`。`[]` 表示这轮没有登记产物，**不等于这轮没有产出文件**（见 [5.7 产物](#_5-7-产物) 开头）；`null` 表示没有记录——平台升级前的历史 run，以及还没跑到终局的 run，都没有清单。**没消费到 `end` 事件就发现 run 已终局的重连场景，从这里拿清单重建，不要用工作区文件状态倒推** |
...
::: warning 一次 run 可能产出了文件却没有登记任何产物
「挑出来交付」是 Agent 的判断，不是平台的自动行为。一次 run 完全可能写出了文件、
而 `end` 事件的 `artifacts` 是 `[]`——例如 Agent 产生的是还要继续加工的中间结果，
按它自己的交付规则不算成品；或者这一轮的计划没有走到交付那一步。

这些文件仍然在 [5.6 工作区文件](#_5-6-工作区文件) 里，可以列出并下载。

**如果客户端的交付链路以产物清单为唯一入口，这类 run 在客户端看来会是「什么都没
...
除了调用方主动删除，产物还有一条自动清理规则：超过保留期没有更新的产物会自动从列表里移出，再过一段宽限期，它的登记信息会被清除。两个期限由平台配置，默认分别是 90 天与 60 天。
...
::: warning limit 的单位是轮，不是条目

**一轮能产出多少条目，事先无法预估。** 实测区间是 6 到 72 条：Agent 追问一句就结束的那种轮次只有几条，真正干完一件事的轮次几十条，取决于它这一轮走了多少步工具。

所以 `limit=20` 可能返回上千个条目。按条目数去规划分页——例如「一页 20 条，那 100 条就翻 5 页」——得到的数字与实际相差一个数量级。

由此推出两条：

- **要不要翻页取决于轮数，与条目数无关。** 一段只有一轮但产出 72 个条目的会话，`limit=5` 也是一页拿完（`has_more` 为 `false`）；一段有 30 轮的会话，哪怕每轮只有几条，也要翻两页。
- **界面上「加载更早」的必要性，按这段会话预计会累积多少轮来判断。** 同一个终端用户反复就同一件事追加对话，轮数就会涨上去。

判断有没有下一页只看 `has_more`，不要拿这一页的条目数去比 `limit`——两者单位不同，比不了。
:::
