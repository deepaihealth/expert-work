...
让已经结束的最后一轮重来一次（同一条输入再跑一次、或者改掉输入再跑一次）不在本章，见 [2.10 重新生成与编辑重发](./chat#_2-10-重新生成与编辑重发)。
...
会话里有 run 停在 `paused` 时，调用方可以照常在同一个会话里发起新一轮（[2.5 多轮会话](./chat#_2-5-多轮会话)，请求体带上同一个 `session_id`）。平台受理这次请求，同时让那条还没有决策的审批失效：

- 那个 run 的状态由 `paused` 变为 `interrupted`，它等待审批的那次工具调用不会执行。
- 之后再对那个 `run_id` 调用本端点，返回 409 `APPROVAL_CONFLICT`，不会续跑。
- 新一轮照常执行。它走到需要审批的一步时，会以自己的 `run_id` 停在 `paused` 并发出新的 `approval` 事件，决策针对这个新的 `run_id` 下达。

`mode` 为 `queue` 的新一轮，在请求被受理时就让待审批失效，这一步不可撤销：即使这个排队的 run 之后被取消，或者因为 Agent 被下线、租户被暂停而没有执行，失效的审批也不会恢复。它排队期间会话里新出现的待审批，在它开始执行时同样失效。

失效的审批在平台的审批记录里显示为已拒绝（`rejected`），处理人记为 `system:new_turn`，租户管理员可以在控制台的审批列表里看到。平台不会为失效另发事件，客户端要发现它，查 [5.4 run 列表](./query#_5-4-run-列表) 里那个 run 的状态，或者照常提交决策并拿到 409。

决策与新一轮几乎同时到达时，平台会检查两次这次决策是否仍然针对会话里正在等待的那次操作：受理决策时检查一次，续跑真正执行之前再检查一次。任何一次发现会话已经进入新一轮，这次决策都不会执行任何操作，也不会放行新一轮里的操作。结果取决于哪一方先被受理：

- 新一轮先被受理：这次决策返回 409 `APPROVAL_CONFLICT`，那次操作不执行。
- 决策先被受理：决策请求返回成功，续跑开始执行；新一轮与续跑在同一个会话里同时进行，两者的输出先后不确定。两者相隔极短时，续跑可能不执行那次操作就结束；如果新一轮这时已经停在自己的审批上，新一轮也会随之结束，它的审批再下达决策时返回 409 `APPROVAL_CONFLICT`，需要重新发起这一轮。
- 新一轮在那次操作仍在执行时到达：新一轮照常开始，但此时那次操作还没有结果，新一轮看到的对话记录里这次调用暂时缺少结果。对调用与结果的配对要求严格的模型服务，可能因此拒绝这一轮及之后各轮的请求。这是已知限制。

**要确认那次操作是否执行过，看续跑事件流里这次调用对应的结果消息**，不要只根据决策请求的返回值判断。

::: tip 需要保留那次操作时先下达决策
要让等待审批的那次操作执行，先下达决策，等续跑事件流的 `end` 到达之后再发起新一轮。这样也能避开上面列出的已知限制。
:::
...
| 409 | `APPROVAL_CONFLICT` | 这条审批已经被决定过，或者与另一次并发决策竞争后落败，且这次请求的 `idempotency_key` 与已记录的那次决策对不上；同一会话里发起新一轮而失效的审批也返回这个码（见上文「决策之前发起新一轮」） | 不要重复决策；要重放上一次的结果，带上当时用的 `idempotency_key` 重新请求 |
