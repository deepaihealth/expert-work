...
`mode` 为 `queue` 的新一轮，在请求被受理时就让待审批失效；它排队期间会话里新出现的待审批，在它开始执行时同样失效。平台不会为失效另发事件，客户端要发现它，查 [5.4 run 列表](./query#_5-4-run-列表) 里那个 run 的状态，或者照常提交决策并拿到 409。

决策与新一轮几乎同时到达时，先被受理的一方生效。决策先生效时，续跑照常执行，新一轮与续跑在同一个会话里同时进行；新一轮先生效时，这次决策返回 409 `APPROVAL_CONFLICT`。
