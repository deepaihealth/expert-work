...
决策与新一轮几乎同时到达时，平台保证这次决策只作用于它针对的那次操作，不会放行新一轮里的任何操作。结果取决于哪一方先被受理：

- 新一轮先被受理：这次决策返回 409 `APPROVAL_CONFLICT`，那次操作不执行。
- 决策先被受理：决策请求返回成功，续跑开始执行；新一轮与续跑在同一个会话里同时进行，两者的输出先后不确定。两者相隔极短时，续跑可能在执行那次操作之前就已结束。

**要确认那次操作是否执行过，看续跑事件流里这次调用对应的结果消息**，不要只根据决策请求的返回值判断。

::: tip 需要保留那次操作时先下达决策
要让等待审批的那次操作执行，先下达决策，等续跑事件流的 `end` 到达之后再发起新一轮。
:::
