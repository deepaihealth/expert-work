...
归属：这三类文件都按租户、终端用户、Agent 三个维度存储。用哪个 `agent_code` 请求，看到的就是那个 Agent 名下的那一份；跨 Agent 的下载与删除返回 404，与「不存在」无法区分。产物列表与工作区文件列表支持 `scope=user`，一次列出这个终端用户名下全部 Agent 的条目，每条带 `agent_code` 标明归属。
