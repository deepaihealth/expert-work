...
- 目录的 `available` 是实时读的,而 run 端点的禁用检查走 30 秒缓存——一个 agent **刚被重新启用**后的最长 30 秒内,可能出现目录已经 `available: true` 但发 run 仍被拒的窗口(有界、会自愈,重试即可;反方向——刚禁用——没有这个窗口)。**不要把目录的 `available` 当成发 run 前的强校验**,该发就发,按返回的错误码处理。
