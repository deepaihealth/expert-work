...
- 目录的 `available` 是实时读的,而 run 端点的禁用检查走 30 秒缓存——**两个方向都可能有最长 30 秒的不一致**:刚**重新启用**后,目录可能已经 `available: true` 但发 run 仍被拒;刚**禁用**后,目录可能已经 `available: false` 但发 run 仍被接受。两个方向都有界、会自愈,重试(或等一小会儿)即可。**不要把目录的 `available` 当成发 run 前的强校验**,该发就发,按返回的错误码处理。
...
`error` 是一段失败诊断文本,只在失败时非空。**run 在流式执行过程中失败**,这个字段与该次 run 在 SSE 事件流里发出的 `error` 帧携带的 `message` 是同一个字符串(两者写自同一处)。**run 还没真正开始执行就失败**(比如排队时找不到对应的 agent),或者**执行它的那个后台实例失联、被接管收尾**,这两种情况不会有对应的 SSE `error` 帧,`error` 是一段固定的平台文案。不管哪种情况,它都**不是**另外一套结构化的错误码体系,不要按文案内容做模式匹配去反推分类(比如"是不是超时""是不是上游拒绝")。
