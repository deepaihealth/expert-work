...
API Key 是持有者凭证：拿到它的任何一方都能以调用方的服务账号身份发起调用、创建终端用户、消耗调用方的配额。**不要把 key 打包进浏览器 JS、移动端 App，或任何终端用户能反编译、抓包取得的地方。**
...
`user_id` 是终端用户在调用方业务里的标识，首次出现时会自动创建一个对应的终端用户。长期记忆、工作区文件、按用户维度统计的用量都归属于这个终端用户。
...
- 给外部对接方的 key 只给 `write` 一档，纯只读集成给 `read`，不要给 `admin`。权限档位越低，key 泄露时的影响面越小，三档的差别见 [6.3 三档权限怎么选](./auth#_6-3-三档权限怎么选)。
...
建议把读超时设为 45 秒：服务端每空闲 15 秒发一行心跳，45 秒相当于连续三个心跳周期没有任何数据。这个建议值以及完整的重连步骤，包括如何找回 `run_id`、如何用事件接口续上，见 [3.6 断线重连](./sse-events#_3-6-断线重连与回放分页)。
...
- [ ] 模拟一次断线，用事件接口接回同一个 run —— [3.6](./sse-events#_3-6-断线重连与回放分页)
