...
流式调用是一条长连接，中间可能因为网络抖动或代理超时断掉。**多久没收到任何数据就判定连接已死，服务端没有规定，得你自己设**——建议 45 秒(服务端每空闲 15 秒发一次心跳，45 秒就是连着三个心跳周期没动静)。这个建议值、以及完整的重连策略——怎么找回 `run_id`、怎么用事件回放接口续上——见 [3.6 断线重连与回放分页](./sse-events#_3-6-断线重连与回放分页)。
...
- [ ] 模拟一次断线，用事件回放接口接回同一个 run —— [3.6](./sse-events#_3-6-断线重连与回放分页)
