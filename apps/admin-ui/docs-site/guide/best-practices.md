...
被取代的那一轮不会被删除，历史消息与对话条目照常返回它，只是每条都带上 `superseded_by`，值是新那一轮的 `run_id`。界面上把带这个字段的消息折叠或者划掉即可，Agent 在后续对话里已经看不见它。

被取代那一轮的用量照常计费，它写过的工作区文件与登记过的产物也不会回退。见 [2.10 重新生成与编辑重发](./chat#_2-10-重新生成与编辑重发)。
...
- [ ] 界面可能中途切走的客户端：切走再回来，确认能用 `since_seq` 接回同一轮、内容不缺 —— [3.6](./sse-events#_3-6-断线重连与续传)
...
- [ ] 需要按 run 记账的客户端:核对一次 `end` 事件里的 `usage_by_model`,再用 run 用量接口取同一个 run 复查,确认两处一致 —— [3.4 的 `end`](./sse-events#end)、[5.9](./query#_5-9-run-用量)
- [ ] 以产物清单驱动交付展示的客户端:跑一次 `artifacts` 为空的 run,确认界面不会把它当成「什么都没发生」——文件可能在工作区里 —— [5.7](./query#_5-7-产物)、[5.6](./query#_5-6-工作区文件)
...
- [ ] 对一轮回答打一次 `down` 并再打一次 `up`，确认第二次响应里 `updated` 为 `true`，且历史消息里只剩最后一票 —— [2.9](./chat#_2-9-给一轮回答打分)
