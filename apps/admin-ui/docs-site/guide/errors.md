...
「端点」列写明这个错误码只可能来自哪些接口；写「全部端点」的表示任何一条对外接口都可能返回。多数错误码链接到本章对应小节；取消 run 与审批决策两个端点的错误码详解在 [4 对话过程中的控制](./run-control)，产物与幂等相关的几个码分别链接到 [5.7 产物](./query#_5-7-产物) 与 [7.7 幂等性](./conventions#_7-7-幂等性)。重新生成与编辑重发这两个端点本身的规则见 [2.10 重新生成与编辑重发](./chat#_2-10-重新生成与编辑重发)。
...
- 请求路径匹配不到任何接口时的 404，全部端点通用，见 [8.6](#_8-6-404-目标不存在)。
- 权限检查不通过的 403，全部端点通用。
...
服务端不区分「对象不存在」与「对象存在但不属于这个调用方」，两种情况返回同一个 404。这是为了不泄露其它租户或其它终端用户的对象是否存在，不是缺陷。

还有一种 404 不带错误码：请求路径拼错，或者路径参数里带了会被当成路径分隔符的字符（例如把 `/` 编码成 `%2F` 放进 `{upload_id}`），请求匹配不到任何接口，返回的是简易格式，只有一个 `detail` 字段，取值固定为 `Not Found`：

```json [响应 404]
{ "detail": "Not Found" }
```

处理方式：核对请求路径与路径参数的取值。这种响应里没有 `error.code`，不要按下面的错误码分支处理。
...
这两个接口只对一段会话的最后一轮有效，目标不是最后一轮时返回 422 `RUN_NOT_LAST`。旧的那一轮不会被删除，它在历史消息与条目里仍然可以读到，只是带上了「已被取代」的标记；它已经产生的副作用不会撤销，两轮各自计费。完整规则见 [2.10 重新生成与编辑重发](./chat#_2-10-重新生成与编辑重发)。
...
重新生成复用被取代那一轮的 `inputs`。那一轮之后 Agent 不再声明任何模板变量、不再声明那一轮传过的某个键，或者那一轮没传的变量现在是必填时，重新生成同样按上面三种情况返回这个格式，处理方式见 [2.10 重新生成与编辑重发](./chat#_2-10-重新生成与编辑重发)。
