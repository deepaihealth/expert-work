...
重新生成或者编辑重发产生的一轮，事件流与普通的一轮完全一样，没有新增的事件类型：`run_id` 是新的这一轮，`thread_id` 与被取代的那一轮相同。被取代的那一轮不出现在事件流里，只在历史消息与对话条目里带上 `superseded_by` 标记，见 [2.10 重新生成与编辑重发](./chat#_2-10-重新生成与编辑重发)。
...
| `length` | 输出被这次调用的最大长度截断，最大长度包含思考。如果截断后这一步没有可用的回答（正文为空，或者带着工具调用），run 以 `error` 事件结束，`message` 以「模型输出被截断」开头，随后是 `status` 为 `error` 的 `end`；正文可用时照常继续 |
...
| `status` | string | 这次工具调用的结果。取值：`success`（工具正常返回，这是默认值）/ `error`（这次调用失败、被平台拦下或没有执行）。只有这两个取值 |
| `artifact` | object | 工具产出的结构化数据，结构随工具而定，也可能整个字段不存在。能够解析就使用，不能解析就忽略 |
| `additional_kwargs.duration_ms` | integer | 这个工具本身执行了多少毫秒 |

`status` 为 `error` 时，客户端一般不需要按成因分支，统一按「这一步的工具调用失败」显示。唯一的例外是第 5 类：结果文本以 `[not run]` 开头，它不是失败，应显示为「未执行」。常见成因有五类，排查时可以对照：

1. 平台的安全策略拦下了这次调用。
2. 工具本身执行时报错。
3. 模型给出的参数没有通过这个工具的参数校验。
4. 这次调用需要人工审批，审批被拒绝。
5. 同一步里的另一次调用经过了人工审批，这次调用没有执行。还原后的结果文本以 `[not run]` 开头（见 [4.2 审批决策](./run-control#_4-2-审批决策)）。
...
决策之前在同一个会话里发起新一轮，这条审批随即失效，同样没有事件通知，见 [4.2 决策之前发起新一轮](./run-control#决策之前发起新一轮)。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `name` | string | 产物名。拿它调 [5.7 产物](./query#_5-7-产物) 就能取回文件（下载还需要 `user_id`，就是你发起这次 run 时传的那个） |
...
::: warning `artifacts` 为空不等于这轮没有产出
Agent 产生的文件分两种去向：一种直接写进这个终端用户的**工作区**，另一种由 Agent
**主动登记为产物**。`artifacts` 只记录后者。

所以一次 run 完全可能产出了文件、而 `artifacts` 是 `[]`——文件在工作区里，可以用
[5.6 工作区文件](./query#_5-6-工作区文件) 列出并下载。典型情形是 Agent 产生的是
中间结果（例如一份还要继续加工的数据文件），按它自己的交付规则不算成品。

**不要把 `artifacts` 非空当作「这轮对用户有产出」的判定条件。** 如果客户端要据此
...
``` [事件流片段]
event: end
data: {"status":"success","run_id":"5a1315a9-6055-454d-90ac-95e1fa6cb782","artifacts":[]}
```

**判断「这轮交付了什么」只认这份清单**，不要用「工作区里有没有文件」「消息里有没有提到文件名」去倒推——那些都不是契约。收割、计费一类的下游动作建议全部挂在清单上。
