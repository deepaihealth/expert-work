...
- 谁发:服务端通过当前这条 SSE 连接发出。实时连接都会发,即 `mode: "stream"` 的响应流以及 run 未结束时的事件接口;run 已结束后的事件续传不发,因为续传一次性返回、不会挂起等待。
...
| [`updates`](#updates) | 每个步骤完成时,一次 | 用它重建对话内容和工具卡 |
| [`worker`](#worker) | Agent 委托子任务时,开始 / 每步 / 结束各一次 | 在对应的工具卡下挂一条子时间线 |
...
`updates` 与 `worker` 两节多几个小节:前者的 `data` 随步骤变化,后者的 `data` 随 `kind` 变化。
...
- 调用方:这次 run 使用 `mode: "queue"`,响应是 202,本身不是 SSE 流;要取得逐字预览只能在 run 未结束时接上事件接口,run 结束后的续传不发 `token`。
...
围栏在不在意味着这段内容是否来自外部、是否可信。建议像上面那样在还原之前先记一个标志位,界面上给这类内容加一个「来自外部工具」的角标,而不是只留下还原后的文本。
:::
...
子任务是 Agent 把一部分工作交给另一个 Agent 去做的机制。委托由模型自己发起:它调用了一个会派生子任务的工具,可能是 Agent 配置里声明的静态子 Agent 工具,也可能是内建的动态派子任务工具。子任务开始执行后,服务端把它的开始、每一步、结束各推送一次到当前这条连接。

三条需要知道的事实:

- 「每一步」是子任务自己的步。子任务每完成一个步骤发一条 `kind` 为 `update` 的事件,与父 run 走到第几步无关。
- 配对依靠 `parent_tool_call_id`。子任务就是那一次工具调用的执行体,所以这个值与 `updates` 里 `ai.tool_calls[].id` 相同,客户端据此把子时间线挂到对应的工具卡下面。
...
| `max_steps` | integer | 这个子任务允许执行的最大步数 |
...
| `_duration_ms` | integer | 距这个子任务上一个事件经过的毫秒数 |
| `step_count` | integer | 到这一步为止的步数。只有 `node` 为 `agent` 的事件带此字段 |
...
摘要按 `type` 分三种结构:`ai` 是 `{type, content_excerpt}`,只有这一步真的发起了工具调用时才多一个 `tool_calls: [{name, args_excerpt}]`;`tool` 是 `{type, name, tool_result_excerpt}`,沙箱执行类工具还多一个 `exec: {exit_code, timed_out, stdout_excerpt, stderr_excerpt}`;其余 `type` 是 `{type, content_excerpt}`。
:::
...
| `iteration_used` | integer | 实际用掉的步数 |
| `llm_call_count` | integer | 这个子任务内部发起的模型调用次数 |
| `wall_clock_ms` | integer | 这个子任务从开始到结束的墙钟耗时,单位毫秒 |
...
平台的三道限制预警或者真正触发时,服务端发一次到当前这条连接。三道限制是:步数上限、token 预算、连续多步没有实质进展。
...
| `tokens_before` | integer | 压缩前的上下文大小,是估算值 |
| `tokens_after` | integer | 压缩后的上下文大小,是估算值,与 `tokens_before` 采用同一种估算方式 |
| `summary_chars` | integer | 结果里那条摘要的字符数,没有摘要时是 `0` |
...
1. **`mode: "queue"` 的 run**——`POST` 直接返回 `202`,没有流,两种模式的差别见 [2.4 stream 还是 queue](./chat#_2-4-stream-还是-queue)。要读取事件需要调用这条接口。
...
`user_id` 必填,而且必须是发起这次 run 的那一个,否则返回 `404`。`since_seq` 可以不带,后果见下面的提示;带的话必须大于等于 `0`,负数返回 `422`。
...
因此客户端必须自行设置一个读超时,3.5 的骨架里是 `readTimeoutMs`。服务端没有规定「多久没有数据算这条连接已经断开」,这个阈值由客户端自行决定;建议设为 45 秒,即连着三个心跳周期一个字节都没有到达,心跳间隔见 3.2。这是建议值,不是协议约束,网络条件较差时可以调大。
:::
...
::: tip 什么时候该校验 seq 连续性
run 还在执行时不要校验:补发的事件可能乱序到达,连续性要等流结束之后再算。完整续传上可以校验,而且命中了就是真的缺失——一次完整续传(不带 `since_seq`)返回的 seq 应当是连续的,出现跳号意味着那几条从来没有被记录,而这种情形不会发送 `gap`。
