...
`updates` 和 `worker` 的 `data` 形状分别随节点、随 `kind` 变，所以这两节在「`data` 字段」和「完整示例」之间多几个小节，把每一种形状单独摆一张表。
...
- **你自己**:这次 run 用的是 `mode: "queue"`(响应是 202，事件要去回放接口拿，回放不发 `token`)。
...
run 记录里的状态比这里细(见 [5.4 run 列表](./query#_5-4-run-列表))，SSE 的 `end` 把它们**收敛成上面这四个**——同一次 run 在两个地方看到不同的字样是正常的。收敛有两处:run 记录里的「已取消」在这里叫 `interrupted`;run 记录里的「超时」和「步数用尽」在这里都叫 `error`。
...
超时之后重新发起同一条 `GET` 重连，**而不是重新调 `POST .../runs`**——那会开启新的一轮 run。
