...
`status` 为 `error` 时，除第 5 类外，客户端不需要按成因分支，统一按「这一步的工具调用失败」显示。常见成因有五类，排查时可以对照：

1. 平台的安全策略拦下了这次调用。
2. 工具本身执行时报错。
3. 模型给出的参数没有通过这个工具的参数校验。
4. 这次调用需要人工审批，审批被拒绝。
5. 同一步里的另一次调用经过了人工审批，这次调用没有执行。这不是失败：还原后的结果文本以 `[not run]` 开头，客户端可据此显示为「未执行」（见 [4.2 审批决策](./run-control#_4-2-审批决策)）。
