...
**除 `end` / `truncated` / `gap` / `token` 这四种帧外,每一帧都带 `id:`**——不管是实时连接直接收到的,还是断线后回放拿到的,同一帧的 id 都是同一个值。
...
- 因此:**不要用"seq 必须连续"来做完整性校验**——回放里 seq 序列本来就允许不连续,这么校验会在正常情况下误报丢帧。
...
客户端的处置:拿 `next_seq` 当下一次请求的 `since_seq`,再发一次同样的请求,把各页的帧拼起来,**直到收到 `end` 为止**。
...
传一个超过这个 run 真实尾部的值时,**服务端不会报错**,而是安安静静什么都不发——你只会收到心跳,以及 run 走到终态时的那条 `end`。看起来就像"这个 run 没有任何事件",非常难查。
