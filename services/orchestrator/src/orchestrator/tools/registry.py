...
#: - ``figure_documents`` —— B-64 ``read_page`` 记下 ``<doc-sha>`` → 路径
#:   (渲染页段的退役占位靠它给出拿回来的路径;读的一侧正向重算哈希再采信)。
TOOL_ALLOWED_STATE_KEYS: frozenset[str] = frozenset(
    {"plan", "subagent_invocations", "promoted_tools", "viewed_figures", "figure_documents"}
)
