...
:class:`AuditBackupSettings` (W2-PR1 Task 1 — OSS S3 compat)."""
...
def test_addressing_style_defaults_to_path() -> None:
    ...
    settings = AuditBackupSettings(_env_file=None)  # type: ignore[call-arg]
...
def test_addressing_style_explicit_field_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("EXPERT_WORK_AUDIT_BACKUP_S3_ADDRESSING_STYLE", "virtual")
    settings = AuditBackupSettings(_env_file=None)  # type: ignore[call-arg]
...
def test_legacy_bool_env_false_maps_to_auto(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    change behavior."""
    monkeypatch.setenv("EXPERT_WORK_AUDIT_BACKUP_S3_USE_PATH_STYLE", "false")
    settings = AuditBackupSettings(_env_file=None)  # type: ignore[call-arg]
...
def test_legacy_bool_env_true_maps_to_path(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("EXPERT_WORK_AUDIT_BACKUP_S3_USE_PATH_STYLE", "true")
    settings = AuditBackupSettings(_env_file=None)  # type: ignore[call-arg]
    assert settings.effective_s3_addressing_style == "path"
