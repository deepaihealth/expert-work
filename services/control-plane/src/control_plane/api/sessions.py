...
def build_sessions_router() -> APIRouter:
    ...
    @router.get(
        "/{thread_id}", dependencies=[Depends(require_key_scope("read")), Depends(console_only())]
    )
    ...
    @router.get("", dependencies=[Depends(require_key_scope("read")), Depends(console_only())])
