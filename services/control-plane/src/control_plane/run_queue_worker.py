...
from control_plane.approval_void import close_previous_turn
...
class RunQueueWorker:
    ...
    async def _bind_exec_trace(self, run: RunInfo) -> None:
        ...
        为什么非做不可、以及三个执行入口各自的情况,见
        :mod:`control_plane.run_trace`。``current_trace_id_hex()`` 在本模块
        取(不下沉到助手里),打桩缝留在这一层。
        """
    ...
    async def _execute(self, run: RunInfo) -> None:
        ...
        with (
            ...
            # B-58 —— 还原搬进 ``graph_input_from_enqueued``,孤儿重收要用同一份。
            # 这段有静默丢附件的前科(document_names / image_refs 各漏过一次回读),
            # 再抄一份等于再开一次同样的口子。
            graph_input = graph_input_from_enqueued(built, payload, run.run_id)

            # 班车 2 —— 出队这一侧再收口一次:入队时会话可能还没停在审批上,
            # 入队与出队之间停下的那条审批只有这里能作废。这时行已经被认领成
            # running,失败必须走 ``_fail`` 收成 error,不能留下一行没有执行者的
            # running(孤儿扫描会拿 graph_input=None 把它「续」成一次空跑)。
            try:
                ...
                logger.exception(
                    "run_queue_worker.close_previous_turn_failed run_id=%s", run.run_id
                )
                await self._fail(run, reason="previous_turn_unsettled")
                return
