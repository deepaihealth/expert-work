"""Endpoint tests for ``/v1/platform/delegation-config`` — perf phase2 PR3.

Mirrors ``test_platform_dynamic_worker_config_api.py``: the full ``create_app``
harness wires ``platform_delegation_config_service`` onto ``app.state``; a
...
async def test_non_admin_forbidden(admin_client: tuple[AsyncClient, UUID]) -> None:
    ...
    resp = await client.get("/v1/platform/delegation-config", headers=_headers(uuid4()))
...
async def test_put_non_admin_forbidden(admin_client: tuple[AsyncClient, UUID]) -> None:
    ...
    resp = await client.put(
        "/v1/platform/delegation-config",
        headers=_headers(uuid4()),
        json={"max_concurrent_delegations": 5},
    )
    assert resp.status_code == 403
    assert resp.json()["detail"]["code"] == "PLATFORM_SCOPE_FORBIDDEN"
...
async def test_get_unset_uses_env_default(admin_client: tuple[AsyncClient, UUID]) -> None:
    ...
    resp = await client.get("/v1/platform/delegation-config", headers=_headers(admin))
...
async def test_put_then_get_reflects(admin_client: tuple[AsyncClient, UUID]) -> None:
    ...
    expected = {"max_concurrent_delegations": 5}
    assert put_resp.json()["data"] == {"configured": expected, "effective": expected}

    get_resp = await client.get("/v1/platform/delegation-config", headers=_headers(admin))
...
async def test_put_rejects_out_of_bounds(admin_client: tuple[AsyncClient, UUID]) -> None:
    ...
    resp = await client.put(
        "/v1/platform/delegation-config",
        headers=_headers(admin),
        json={"max_concurrent_delegations": 65},
    )
    ...
    resp = await client.put(
        "/v1/platform/delegation-config",
        headers=_headers(admin),
        json={"max_concurrent_delegations": 0},
    )
    assert resp.status_code == 422
...
async def test_put_rejects_unknown_field(admin_client: tuple[AsyncClient, UUID]) -> None:
    ...
    resp = await client.put(
        "/v1/platform/delegation-config",
        headers=_headers(admin),
        json={"max_concurrent_delegations": 5, "bogus": 1},
    )
    assert resp.status_code == 422
...
async def test_put_emits_audit(admin_client: tuple[AsyncClient, UUID]) -> None:
    ...
    matched = [e for e in entries if e.action.value == "platform_delegation_config:updated"]
    assert matched, "expected a PLATFORM_DELEGATION_UPDATED audit row"
    assert matched[0].details == {"max_concurrent_delegations": 5}
...
@pytest.mark.asyncio
async def test_put_broadcasts_platform_delegation(
    settings: Settings,
    lifecycle: Lifecycle,
    jwt_verifier: JWTVerifier,
) -> None:
    """``put`` self-invalidates THIS pod's cache; the endpoint must also
    broadcast ``platform_delegation`` so peer replicas drop theirs."""
    app = create_app(settings=settings, lifecycle=lifecycle, jwt_verifier=jwt_verifier)
    ...
    assert spy_bus.events[0].kind == "platform_delegation"  # type: ignore[attr-defined]
