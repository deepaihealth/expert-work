...
@pytest.mark.asyncio
async def test_latest_by_thread_picks_newest_scoped_and_can_skip_one(
    run_store: SqlRunStore,
) -> None:
    ...
    latest = await run_store.latest_by_thread(thread_id=thread_id, tenant_id=tenant_a)
    assert latest is not None and latest.run_id == newest
    skipped = await run_store.latest_by_thread(
